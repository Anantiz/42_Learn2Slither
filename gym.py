...
def interupt_save_model(sig, frame):
    if MODEL is not None and IS_TRAINING:
        print(
            f"{colors.RED}Interupt signal received, saving model{colors.RESET}"
        )
        MODEL.save_weights()
    exit(0)
...
class Gym:
    ...
    def __init__(self, brain, sim_generator: callable, cpu_count: int = 1):
        ...
        assert (
            0
            < cpu_count
            <= (os.cpu_count() if os.cpu_count() is not None else cpu_count)
        ...
        self.cpu_count = (
            cpu_count  # Hopefully you won't rip a core out while running this
        )
        self.brain = brain

    def run_episode(
        self, agent, sim
    ) -> list[tuple[Tensor, int, float, Tensor]]:
        """
        ...
        while True:
            ...
            # s, a, r, s', done
            buff.append(
                (prev_state[0], prev_state[1], prev_state[2], state, done)
            )
            if done:
                break
        ...
        if len(buff) == 500 and sim.snake_len < 5:
            return (
                []
            )  # This is just useless data of the snake going in circles, it will reinforce bad behavior if used
        return buff

    def _parallel_worker(
        self, agent, sim, requested_experiences_count, shared_experiences, lock
    ):
        """
    ...
    def _parallel_manager(
        self, sim_pool, batch_size
    ) -> list[tuple[Tensor, int, float, Tensor]]:
    ...
    def train(self, epoch=1000, batch_size=64, save_file=None):
        ...
        if isinstance(self.brain, Maurice):
            ...
            self.batch_size = (
                8  # No need for batches, q-tables have independent updates
            )

        print(
            f"{colors.GREEN}Training for {epoch} epochs, with a batch size of {batch_size}, and {self.cpu_count} workers{colors.RESET}"
        )
        if self.cpu_count > 1:
        ...
        for e in range(epoch):
            while len(experiences) < batch_size * batch_count:
                if self.cpu_count == 1:
                    ...
                    experiences.extend(
                        self._parallel_manager(sim_pool, batch_size * 2)
                    )
            if e % mod == 0:
                average_reward = sum([exp[2] for exp in experiences]) / len(
                    experiences
                )
                print(
    ...
    def test(self, cli_map=False, max_tick=1500, render_speed=None):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                r += sim.step(
                    self.brain.qna(s, learning_on=False), max_tick=max_tick
                )
                max_len = max(max_len, sim.snake_len)
        ticks = sim.ticks
        snake_len = sim.snake_len
        print(
            f"{colors.CYAN}Simulation ended after {ticks} ticks, with a size of {snake_len}, average reward {r/ticks}{colors.RESET}"
        )
    ...
    def test_record(
        ...
        while redo:
            ...
            redo = False
            if (
                len(frames) < min_accepted_tick
                or max_len < min_acepted_snake_len
            ):
                redo = True
                frames.clear()
                if max_retries == 0:
        ...
        try:
            ...
            return None
        print(
            f"{colors.CYAN}Record saved as: {record_file_path}{colors.RESET}"
        )
        print(
